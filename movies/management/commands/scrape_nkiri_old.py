...
API_POSTS_URL = 'https://thenkiri.ng/wp-json/wp/v2/posts/'
API_PAGES_URL = 'https://thenkiri.ng/wp-json/wp/v2/pages/'
API_URL       = API_POSTS_URL  # default; overridden by --source flag
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--source', type=str, default='both',
                            ...
                            help='Which WP endpoint to scrape: posts, pages, or both (default: both)')
    ...
    def handle(self, *args, **options):
        ...
        if no_social:
            print("\U0001f507 --no-social: DB save only, no social posts")
        if telegram_only:
            print("\U0001f4e2 --telegram-only: Telegram posts only")
        ...
        if source in ('posts', 'both'):
            endpoints.append(('posts', API_POSTS_URL))
        if source in ('pages', 'both'):
            endpoints.append(('pages', API_PAGES_URL))

        for source_name, api_url in endpoints:
            print(f"\n{'\u2550'*60}")
            print(f"\U0001f4e1 Scraping WP endpoint: {source_name.upper()} \u2192 {api_url}")
            print(f"{'\u2550'*60}")
    ...
    def _scrape_endpoint(self, api_url, start_page, end_page, max_pages, no_social, telegram_only):
        from django.db import connection
        ...
        print(f"\U0001f680 Starting scrape from page {start_page}")
        if end_page:
            print(f"\U0001f4c4 Will stop at page {end_page}")
        if max_pages:
            print(f"\U0001f4ca Will scrape maximum {max_pages} pages")
        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 404:
                    ...
                    break
                print(f"\U0001f525 HTTP error: {http_err}")
                ...
                print(f"\U0001f525 Request failed: {e}")
                consecutive_errors += 1
                if consecutive_errors >= max_consecutive_errors:
                    print(f"\u274c Too many consecutive errors. Stopping.")
                    return
            ...
            for item in data:
                ...
                try:
                    ...
                    if not created and not updated and added == 0 and updated_labels == 0 and deleted == 0:
                        print("ℹ️ No updates.")
